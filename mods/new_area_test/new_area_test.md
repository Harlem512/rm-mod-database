...
match room_get() {
  case rm_start_0 {
    if !global.__na_test_init {
      ...
      ds_grid_set(global.map_grid_, 0, 0, global.__test_room)
    }
...
```sp
draw_text(0, 0, string([room_get(), room_width_get(), room_height_get()]))

with oplayer {
  draw_text(0, 10, string([self.x, self.y]))
}
```
